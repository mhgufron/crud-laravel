...
<form action="{{ route('santri.update', Santri->id) }}" method="post">
    ...
    @csrf
    <input type="text" name="nama" placeholder="Edit Name" value=""><br>
    @if ($errors->has('nama'))
        <strong>{{ $errors->first('nama') }}</strong><br>
    @endif
    <input type="text" name="umur" placeholder="Edit Umur" value=""><br>
    @if ($errors->has('umur'))
        <strong>{{ $errors->first('umur') }}</strong><br>
    @endif
    <textarea name="alamat" rows="5" cols="30" placeholder="Edit Address" value=""></textarea><br>
    @if ($errors->has('alamat'))
        <strong>{{ $errors->first('alamat') }}</strong><br>
    @endif
    <input type="text" name="jenis_kelamin" placeholder="Edit Gender" value=""><br>
